...
public class AutoAttackEvaluator : MonoBehaviour
{
    ...
    public static ConcurrentDictionary<GameObject, AttackerParallelnfo>
    AttackersInfo = new ConcurrentDictionary<GameObject, AttackerParallelnfo>();
    public static ConcurrentDictionary<GameObject,
    FactionMemberParallelInfo> FactionMembersInfo = new
    ConcurrentDictionary<GameObject, FactionMemberParallelInfo>();
    ...
    private void evaluate(GameObject go, AttackerParallelnfo info)
    {
        ...
        foreach (var (otherGo, otherFactionInfo) in FactionMembersInfo)
        {
            ...
            var distance = Vector3.Distance(factionInfo.Position,
            otherFactionInfo.Position);
